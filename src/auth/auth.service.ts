...
export class AuthService {
    ...
    private async sendResetPasswordEmail(email: string, resetUrl: string) {
        const gmailUser = this.config.get<string>('GMAIL_USER') || this.config.get<string>('MAIL_USER');
        const gmailPass = this.config.get<string>('GMAIL_APP_PASSWORD') || this.config.get<string>('MAIL_PASS');
        const fromEmail = this.config.get<string>('MAIL_FROM') || gmailUser;

        if (!gmailUser || !gmailPass || !fromEmail) {
            throw new BadRequestException('Email service is not configured. Please set GMAIL_USER and GMAIL_APP_PASSWORD.');
        }
        ...
        try {
            ...
            throw new BadRequestException('Failed to send reset email. Check Gmail credentials and App Password.');
        }
    }
    ...
    async resetPassword(token: string, newPassword: string) {
        ...
        const tokenHash = this.hashResetToken(token);
        const user = await this.usersService.findByResetPasswordTokenHash(tokenHash);
        ...
        await this.usersService.updatePasswordByUserId(String(user._id), passwordHash);
